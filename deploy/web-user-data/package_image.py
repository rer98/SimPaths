...
Create a fresh Docker context for UK user-data SingleRun with inactive bundled examples.
...
def package(repo, output):
    ...
    with zipfile.ZipFile(jar) as archive:
        if not {'simpaths/experiment/SimPathsUserDataStartup.class',
    ...
    deployment = repo / 'deploy/web-user-data'
    ...
    if shutil.disk_usage(ancestor).free < size:
        raise ValueError(f'Context requires at least {size / 1024**3:.2f} GiB free, plus Docker image space')
    output.mkdir(parents=True)
    for source, relative in copies:
        target = output / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target)
    print(f'Context ready: {output}\nStartup preparation runs separately with a 3 GiB heap. Allow memory for both JVMs.')
